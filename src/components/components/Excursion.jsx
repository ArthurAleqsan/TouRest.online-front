...
const Excursion = memo(({ location, tour, disabled, handleClick, openTourPage, fromCart, lng, openImageModal }) => {
            ...
            <div className='tours-container-slider'>
                ...
                >
                    {tour.images && tour.images.map(img => {
                        return <div key={img} className='tours-container-slider-image' onClick={() => openImageModal(tour.images)}>
                ...
                <div className='single-excursion-container' >
                    <div className='desc-column' onClick={openTourPage}>
                        ...
                        {fromCart ? <div className='excursion-desc-str-cartpage'>
                            ...
                            <span className='excursion-price'> {tour.peopleCount}</span>
                        </div>
                            : <div className='excursion-desc-str'>
                                <Rate allowHalf value={makeRoundedRate(rate)} disabled={disabled} />
                            </div>}
                        {fromCart && tour.childCount !== 0 && <div className='excursion-desc-str-cartpage'>
                            <span className='excursion-desc-span'>{t('Ticket count for children')} : </span>
                            <span className='excursion-price'> {tour.childCount}</span>
                        </div>}
                        {fromCart && <Fragment>
                            <div className='excursion-desc-str-cartpage'>
                                <span className='excursion-desc-span'>{t('Tour is starting at')} : </span>
                                <span className='excursion-price'> {moment(new Date(tour.firstDate)).format('DD/MM - HH:mm')}</span>
                            </div>
                            <div className='excursion-desc-str-cartpage'>
                                <span className='excursion-desc-span'>{t('Tour duration')} : </span>
                                <span className='excursion-price'> {isNaN(+tour.duration) ? tour.duration : `${tour.duration} hour`}</span>
                            </div>
                        </Fragment>}
                        <div className='excursion-desc-str '>
                            {fromCart ? (
                                <button
                                    className={'remove-btn link btn btn-style'}
                                    onClick={handleClick}
                                >
                                    {t('Remove')}
                                </button>
                            ) : (
                                    <Link to={`${location}/${lng}/tour-booking`}>
                                        <button
                                            className='btn link btn-style'
                                        >
                                            {t('Add to cart')}
                                        </button>
                                    </Link>
                        ...
                        {!fromCart &&
                            <div className='excursion-desc-str'>
                                ...
                                <span className='excursion-desc-span link'> {t('See More')}</span>
                            </div>
                        }
                    </div>
                </div>
